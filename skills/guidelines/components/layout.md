...
**Compound sub-components (verified in @contentful/f36-layout source):**

| Sub-component    | Renders as | Purpose                                                              |
| ---------------- | ---------- | -------------------------------------------------------------------- |
| `Layout.Header`  | `<header>` | Wraps the `Header` component; pass via the `header` prop             |
| `Layout.Body`    | `<div>`    | Main content wrapper; handles overflow and scrolling                 |
| `Layout.Sidebar` | `<aside>`  | Left or right sidebar; pass via `leftSidebar` / `rightSidebar` props |

These are the ONLY valid sub-components. They are real, exported, typed compound components — not fabricated.

```tsx
import { Layout, Header } from '@contentful/f36-components';

// Correct usage — all three sub-components
<Layout
  header={
    <Layout.Header>
      <Header title="Page" />
  ...
  leftSidebar={<Layout.Sidebar>nav here</Layout.Sidebar>}
>
  <Layout.Body>content here</Layout.Body>
</Layout>;
```

**Does NOT exist — never use:**

- ~~`Layout.Content`~~ — use `Layout.Body`
- ~~`Layout.Footer`~~ — no footer sub-component; place footer content inside `Layout.Body`
- ~~`Layout.Main`~~ — use `Layout.Body`
- ~~`Layout.Nav`~~ — use `Layout.Sidebar` with `leftSidebar` prop
