...
- Menu - s a component that offers a list of choices to the user, such as a set of actions or links. Menu uses Popover component as a base.
- Popover - is used to display some content on top of another, and should be paired with a clickable trigger element. It is a base component for Menu, but not styled and not handled. If customer need to create some specific, custom solution, then this would be the right choice
- Autocomplete - component is an input field that provides selectable options as a user types into it. It allows users to quickly search through and select from large collections of options.
