---
title: 'TableNext'
type: 'component'
status: 'alpha'
slug: /components/tablenext/
github: 'https://github.com/contentful/forma-36/tree/main/packages/components/tablenext'
storybook: 'https://f36-storybook.contentful.com/?path=/info/components-tablenext--default'
typescript: ./src/TableNext.tsx,./src/TableCell/TableCell.tsx,./src/TableRow/TableRow.tsx,./src/TableBody/TableBody.tsx,
---

`TableNext` is the next iteration of the basic `Table` component. It is per default responsive by receiving a horizontal scrollbar. It additionally offers a second responsive layout mode called `stackable`. In the `stackable` layout, the Table data will be displayed in a grid, each table row represented as a single unit with all cells of this row stacked on top of each other. If a table header with titles is required, the column title is displayed next to the table cell.

## Current Limitations

This component is still in an unstable alpha state. At the moment it does not offer the possibility of sorting as well as adding a check-all checkbox in the header.

## Import

```jsx static=true
import { TableNext } from '@contentful/f36-tablenext';
```

## Examples

### Basic usage

```jsx file=./examples/TableBasicUsageExample.tsx

```

### Dynamic creation

One very common use case for a table is that you will have a set of data and you would like to show a table row for each item in that set.
To achieve that result, you can iterate over the data and create `TableNext.Row` and `TableNext.Cell` for each item:

```jsx file=./examples/TableDynamicCreation.tsx

```

### With sticky first column

Combine `layout="scrollable"` with `isFirstColumnSticky` when the first column contains the primary identifier for each row. This keeps the row context visible while users scroll horizontally.

```jsx file=./examples/TableScrollableStickyFirstColumnExample.tsx

```

### With sticky header and first column

For large tables that need both horizontal and vertical scrolling, combine `layout="scrollable"`, `isFirstColumnSticky`, and `TableNext.Head isSticky`. Place the table in a height-constrained container to avoid double scrollbars coming from the outer container.

```jsx file=./examples/TableScrollableStickyHeaderAndFirstColumnExample.tsx

```

### Stacked responsive behaviour with column titles

For containers smaller than `700px` the table will be presented as 90° flipped, stacking all table cells of a single table row underneath each other with column titles next to them.

```jsx file=./examples/StackableWithTitleExample.tsx

```

### Stacked responsive behaviour without titles

For containers smaller than `700px` the table will be presented as 90° flipped, stacking all table cells of a single row underneatheach other.

```jsx file=./examples/StackableExample.tsx

```

## Props (API reference)

### TableNext

<PropsTable of="TableNext" />

### TableNext.Body

<PropsTable of="TableBody" />

### TableNext.Row

<PropsTable of="TableRow" />

### TableNext.Cell

<PropsTable of="TableCell" />

## Content guidelines

- Table Column Titles should be short and descriptive

## Accessibility

- It will render tabular data using the native HTML element `table` which is recommended.
