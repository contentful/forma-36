...
- [Code examples](#code-examples)
  - [Highlighting an item with `getMatch`](#highlighting-an-item-with-getmatch)
  - [Using objects as items](#using-objects-as-items)
  - [Using grouped object as items](#using-grouped-objects-as-items)
...
## Highlighting an item with `getMatch`
...
This is possible by using the `renderItem` prop and the `getMatch` utility function:

```jsx static=true
// We need to import the function from the `f36-autocomplete` package
import { Autocomplete, getMatch } from '@contentful/f36-autocomplete';
...
<Autocomplete
  ...
  renderItem={(item, inputValue) => {
    // we pass `item.name` and the inputValue to getMatch and the util will return an object
...
The most important part of making this work is the shape of the grouped object. The options themselve work exactly as in the object example and require the `itemToString`
and `renderItem` functions.
Besides the correct shape of the object the Autocomplete component needs to receive the prop `isGrouped=true`
...
function AutocompleteExample() {
  ...
  return (
    <Stack flexDirection="column" alignItems="start">
      ...
      <Paragraph>Selected item from shopping list: {selectedItem}</Paragraph>
    </Stack>
  );
}
```
