...
<Table>
  ...
  <Table.Body>
    <Table.Row>
      ...
      <Table.Cell>
        Should contain a `Header` component. When passing at least one sidebar, a divider is added underneath the header. Content inside will be centered. It follows through in the `max-width` adjustment from `1550px` to `1920px` on the `wide` variant.
      </Table.Cell>
    ...
    <Table.Row>
      ...
      <Table.Cell>
        Allows for variants `narrow` (sets `width: 280px`) and `wide` (default, sets `width: 340px`). Both sidebars can have different widths.
      </Table.Cell>
    ...
    <Table.Row>
      ...
      <Table.Cell>
        Wrapper around all children, this compound component is essential for proper overflow and scrolling behaviors.
      </Table.Cell>
