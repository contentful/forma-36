...
### Tooltip without wrapper (`asChild`)

- Use `asChild` to avoid rendering an additional wrapper element around the trigger.
- `asChild` requires a single valid React element as `children`.
- `as` and `targetWrapperClassName` are wrapper-mode props and cannot be used together with `asChild`.

```jsx file=./examples/AsChildTooltipExample.tsx
