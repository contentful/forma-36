...
describe('Tag', () => {
  ...
  describe('badge', () => {
    it('renders badge when provided', () => {
      render(
        <Tag label="test" badge={<span data-test-id="badge">Public</span>} />,
      );
    ...
    it('does not render badge slot when badge is not provided', () => {
      ...
      const badgeWrapper = Array.from(spans).find(
        (el) => el.childElementCount === 0 && el.textContent === '',
      );
      expect(badgeWrapper).toBeUndefined();
    });
    ...
    it('renders badge between label and remove button', () => {
      const { container } = render(
        <Tag
          label="test"
          badge={<span data-test-id="badge">Public</span>}
          onRemove={() => {}}
        />,
      );
      ...
      const badgeIndex = children.findIndex(
        (el) => el.querySelector('[data-test-id="badge"]') !== null,
      );
  ...
  describe('accessibility', () => {
    ...
    it('has no a11y violations with badge and remove button', async () => {
      const { container } = render(
        <Tag label="test" badge={<span>Public</span>} onRemove={() => {}} />,
      );
      const results = await axe(container);
      expect(results).toHaveNoViolations();
    });
