...
typescript: ./src/Navbar.tsx,./src/NavbarAccount/NavbarAccount.tsx,./src/NavbarBadge/NavbarBadge.tsx,./src/NavbarItem/NavbarItem.tsx,./src/NavbarMenuItem/NavbarMenuItem.tsx,./src/NavbarHelp/NavbarSubmenu.tsx,./src/NavbarSwitcher/NavbarSwitcher.tsx,
---

The Navbar component offers a unified navigation experience across all of Contentful's products.

> [!CAUTION]
> The Navbar component is offering a responsive behaviour. Parts of the navbar will dissapear on certain breakpoints. Find the detailed information in the table below.

The Navbar consists of multiple areas, which can be targeted via dedicated navbar props and does not accept `children`. It is responsive and shows / hides different areas on different breakpoints.
...
| `promotions`          | In this area you can add any additional items you want accessible on the Navbar, like feedback links or other types of promotions. This area is responsive and will not be visible until the breakpoint `1200px`.                                                                                                                                                    | `1200px` and up                  | Custom element                      |
...
1. `<Navbar.MenuItem>`: Represents a menu item used to create dropdown menus. Use it as a child of the `<Navbar.Item>` or `<Navbar.Account>` components.
1. `<Navbar.Submenu>`: Used as wrapper around `<Navbar.MenuItem>` to create a nested Menu inside the `mobileNavigation`
