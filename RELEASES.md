...
Our releases are managed automatically though the CI, if a new changeset is present on the PR that was merged into the main branch.
...
It's also updates the [What's new](https://f36.contentful.com/whats-new) page that we show in our website, to do that we had to make a custom flow so we could group all changes by packages and update the website daily.

## Generating What's new page

The [What's new](https://f36.contentful.com/whats-new) page is generated daily, based on the changesets that merged since it was last generated, to manage this we had to create a flow where we store all changesets in a single file from where we later extract the changes to add to the page, that is handled by the CI, on the release job, and uses the [changelog-generate.js](https://github.com/contentful/forma-36/blob/fe934ff657852993ef321348651cbce0a68dc349/scripts/changesets/changelog-generate.js) script file.
...
Daily we have a task that runs on the CI that calls the [changelog-write.js](https://github.com/contentful/forma-36/blob/fe934ff657852993ef321348651cbce0a68dc349/scripts/changesets/changelog-write.js), this file checks for contents on the `.changelogrc` file and generates the [What's new](https://f36.contentful.com/whats-new) page for the website based on those, and empty the files to not have duplicated contents when it needs to generate the page again.
